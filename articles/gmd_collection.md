...
([GMD](https://worldbankgroup.sharepoint.com/:u:/r/sites/WBPoverty/SitePages/PublishingPages/Global-Monitoring-Database-GMD-02062020-093339.aspx?csf=1&web=1&e=dVCBX4))
...
Poverty and Equity Global Practice. The Global Poverty & Inequality Data
Team (GPID) in the Development Economics Data Group (DECDG) also
contributes historical and external data, including from the Luxembourg
Income Study (LIS).

The GMD harmonizes key variables to enable meaningful comparisons of
...
and Inequality Platform ([PIP](https://pip.worldbank.org/home)), the
Multidimensional Poverty Measure (WB MPM), the Global Database of Shared
...
The Datalibweb system is organized into several servers, each hosting
one or more *collections* of data. For example, the GMD collection is
hosted on the GMD server, which can be confusing since both the
collection and the server share the same name.
...
If you do not specify a server, the default is
...
The catalog contains all the metadata required to download any dataset
from the GMD collection, provided you have the necessary access rights.
For example, let’s examine the data available for Paraguay in 2020
...
Notice that there are three different versions available for this
...
function, which is the main function for downloading data in the
...
While specifying all these arguments is necessary, since the Datalibweb
API requires them, it can be tedious. To simplify this process, the
[`dlw_get_gmd()`](https://worldbank.github.io/dlw/reference/dlw_get_gmd.md)
function acts as a convenient wrapper around
[`dlw_get_data()`](https://worldbank.github.io/dlw/reference/dlw_get_data.md).
With
[`dlw_get_gmd()`](https://worldbank.github.io/dlw/reference/dlw_get_gmd.md),
you only need to provide the country code, year, and module, and it will
...
Notice that
[`dlw_get_gmd()`](https://worldbank.github.io/dlw/reference/dlw_get_gmd.md)
prints the actual call to
[`dlw_get_data()`](https://worldbank.github.io/dlw/reference/dlw_get_data.md)
in the console for your reference and verification.

### All versions

By default,
[`dlw_get_gmd()`](https://worldbank.github.io/dlw/reference/dlw_get_gmd.md)
returns the most recent version of the data for a particular year.
However, if you set the argument `latest_version = FALSE`, the function
will instead return a list of calls—one for each survey version
available for that year. This allows you to review and select the
...
If you do not specify the year,
[`dlw_get_gmd()`](https://worldbank.github.io/dlw/reference/dlw_get_gmd.md)
will return calls for all surveys available for the given country and
module. If you want to retrieve only the most recent year (which may
differ by country and module), set `latest_year = TRUE`. This will
...
It is possible that, depending on the country and module, you may need
to access an older version of the data. In such cases, you can specify
the `veralt` argument to indicate which version you want to download.
For example, if you want to download the second version of the Paraguay
